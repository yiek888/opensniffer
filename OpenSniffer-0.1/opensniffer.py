...
try:
    ...
    #address = output[indexBeg:indexEnd]
...
class OpenSniffer:
    ...
    def getInfo(self):
        # Create request
        req = urllib2.Request('http://' + self.IP + '/index.shtml')
        # Open url
        ...
        # Find index of parenthesis of the value variable
        indexEnd = data.find(').split', index) - 1
    ...
    def injectBytes(self, band, repeat=100, payload='010203'):
        ...
        REST = "http://" + self.IP
        # Append channel & modulation
        REST += "/inject.cgi?" + band
        # Append repeat time, txlevel etc.
        REST += "&txlevel=0&rxen=0&nrepeat=" + str(repeat) + "&tspace=1&autocrc=1"
        # Append payload
        REST += "&spayload=" + payload +"&len=" + str(len(payload)/2)
        # Create request
        req = urllib2.Request(REST)
        # Actually open url
        try:
